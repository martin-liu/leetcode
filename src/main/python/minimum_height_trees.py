...
---
Basic Idea: similar like topological sort, keep find and remove leafs, until only 1 or 2 node
"""
        ...
        # no adj(for one node) or only one adj is leaf
        leaves = [i for i in range(n) if len(adj[i]) <= 1]

        # keep remove and find new leaves
        while n > 2:
            n -= len(leaves)
            newLeaves = []
            for i in leaves:
